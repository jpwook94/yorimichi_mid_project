...
function initGachaMachineEvents() {
  ...
  const speechBubble = document.querySelector(
    ".gachacitypick-pickstart-speechbubble"
  );
  const joystickAudio = new Audio(
    "/other/audio/gacha/robot-movement-sound-effects.mp3"
  );
  ...
  pickup.addEventListener("click", async () => {
    ...
    const backlight = document.querySelector(
      ".gachacitypic-picked-mascot-backlight"
    );
    const locationnameEl = document.querySelector(
      ".gachacitypic-picked-locationname"
    );
    if (locationnameEl) {
    ...
    const mascotnameEl = document.querySelector(
      ".gachacitypic-picked-mascotname"
    );
    if (mascotnameEl) {
      mascotnameEl.textContent = data.mascot_name;
      mascotnameEl.setAttribute("data-content", data.mascot_name);
    }
    const pickedmascotimg = document.querySelector(
      ".gachacitypic-picked-mascotimg"
    );
...
function initSSRcardEvents() {
  ...
  const closeButton = document.querySelector(
    ".likegacha-window-controls .likegacha-control-btn:last-child"
  );
  const noButton = document.querySelector('.likegacha-main-btn[value="n"]');
  const closeButton2 = likegachaContainer2.querySelector(
    ".likegacha-control-btn:last-child"
  );
  const noButton2 = likegachaContainer2.querySelector(
    '.likegacha-main-btn[value="n"]'
  );
  const closeButton3 = fivegachaContainer.querySelector(
    ".fivegacha-window-controls:last-child"
  );
  /* 닫기 버튼 누를 때 사운드 */
  ...
  SSRcardpicked1.addEventListener("click", async (e) => {
    ...
    if (likeBtn && data.destination_number) {
      // likeBtn과 data.destination_number가 모두 있는지 확인
      ...
      document
        ...
        .setAttribute("data-destination-number", data.destination_number);
    } else {
      console.warn(
        "likeBtn 요소를 찾을 수 없거나 data.destination_number가 없습니다."
      );
    }
          ...
          setTimeout(() => {
            ...
            setTimeout(() => {
              if (exists == 1) {
                ...
                document.querySelector(".likegacha-container22").style.display =
                  "block";
              } else {
                // 찜 안해둔 여행지면 찜할래?? 윈도우 보여주기
                document.querySelector(".likegacha-container").style.display =
                  "block";
  ...
  function playSSRN(destinations, likedMap = {}) {
    ...
    function revealNext() {
      ...
      if (index >= destinations.length) {
        ...
        document
          .querySelectorAll("input[data-destination-number]")
          .forEach((e) => console.log(e.onclick));
        document.querySelector(".SSRcard-wrapper").classList.add("hide");
        setTimeout(() => {
      ...
      if (wrapper) {
        ...
        const pickeddestimg = document.querySelector(
          ".SSRcard-imgcontainer img"
        );
    ...
    const yesClickHandler = (e) => {
      ...
      if (mode === "single") {
        ...
        const checked = document.querySelectorAll(
          'input[name="fivelike"]:checked:not(:disabled)'
        );
        if (checked.length === 0) {
        ...
        const destinationNums = Array.from(checked).map(
          (input) => input.dataset.destinationNumber
        );
        ...
        if (res.ok) {
          ...
          destinationNumbers.forEach((num) => {
            const input = document.querySelector(
              `input[data-destination-number="${num}"]`
            );
            if (input) {
              input.checked = true;
              input.disabled = true;
